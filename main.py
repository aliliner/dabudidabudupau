...
with st.echo(code_location = 'below'):
    ...
    column_list      = ['Цена', 'Рейтинг', 'Страна', 'Старый свет', 'Возраст', 'Объем', 'Крепость', 'Потенциал мин', 'Виноград #']

    df2 = df1[['price', 'rating', 'Вино', 'Сахар', 'Страна, регион', 'Виноград', 'Крепость', 'Объем', 'Регион', 'Год', 'Потенциал хранения', 'Аппелласьон', 'Категория', 'Декантация']]
    ...
    df2['Цена']        = df2.apply(lambda row: float(re.findall(r'(.*)( ?)(₽)',      row['Цена']    )[0][0].replace(' ','')), axis = 1,  result_type = 'expand')
    df2['Объем']       = df2.apply(lambda row: float(re.findall(r'(.*)( [А-Яа-я]*)', row['Объем']   )[0][0]                ), axis = 1,  result_type = 'expand')
    df2['Рейтинг']     = df2.apply(lambda row: float(                                row['Рейтинг']                        ), axis = 1,  result_type = 'expand')
    df2['Крепость']    = df2.apply(lambda row: float(re.findall(r'(.*)(%[А-Яа-я]*)', row['Крепость'])[0][0]                ), axis = 1,  result_type = 'expand')

    df2['Виноград #']     = df2.apply(lambda row: len(row['Виноград'].split('##')),                                                  axis = 1,  result_type = 'expand')
    df2['Потенциал мин']  = df2.apply(lambda row: int(re.findall(r'([0-9]{1,4})( |-|$|[А-Яа-я])', row['Потенциал хранения'])[0][0]), axis = 1,  result_type = 'expand')
    ...
    df2 = df2[['Цена', 'Рейтинг', 'Вино', 'Сахар', 'Страна', 'Регион', 'Старый свет', 'Год', 'Возраст', 'Объем', 'Крепость', 'Потенциал мин', 'Виноград #', 'Аппелласьон', 'Категория', 'Декантация']]
    ...
    color_discrete_sequence = [ '#EAA148', '#D4514A', '#DF7832', '#CDAF95', '#FFA3A3', '#D64242', '#9A2121',
                                '#D11F3F', '#FFA583', '#D11F3F', '#FFEFD5', '#FF615D', '#FFA583', '#DAD8C0', '#FFDAB9', '#EAF0F1',
    ...
    appellasion_list = ['Bolgheri Sassicaia DOC', 'Pauillac AOC','Cote Rotie AOC', 'Pessac-Leognan AOC', 'Priorat DOC', 'Ribera del Duero DO', 'Chianti Classico DOCG',
    'Napa Valley AVA','Pomerol AOC', "Val d'Arno di Sopra DOC", 'Recioto della Valpolicella DOC', 'Margaux AOC', 'Saint-Estephe AOC', 'Meursault AOC', 'Toscana IGT', 'Rutherford AVA', 'Saint-Julien AOC',
    'Sauternes AOC', 'Montrachet Grand Cru AOC', 'Valle del Aconcagua', 'Santa Cruz Mountains AVA', 'Chambertin Clos de Beze Grand Cru AOC',
    'Barolo DOCG','Romanee Saint-Vivant Grand Cru AOC','Bolgheri Superiore DOC','Saint-Emilion Grand Cru AOC','Pago de Arinzano DO','Willamette Valley AVA','Puligny-Montrachet 1-er Cru AOC',
    'Puligny-Montrachet AOC','Beaune 1-er Cru AOC','Brunello di Montalcino DOCG', 'Corton-Charlemagne Grand Cru AOC','Venezie IGT',
    'Valpolicella Classico DOC','Chambertin Grand Cru AOC', 'Haut-Medoc AOC','Amarone della Valpolicella DOCG', 'Chablis Grand Cru AOC','Lujan de Cuyo',
    'Puente Alto','Marche IGT','Nuits-Saint-Georges 1-er Cru AOC', 'Clos-de-Tart Grand Cru AOC','Clos des Lambrays Grand Cru AOC','Gevrey-Chambertin AOC',
    'Carmignano DOCG', 'Colli della Toscana Centrale IGT', 'Echezeaux Grand Cru AOC', 'Vigneti delle Dolomiti IGT', 'Bourgogne AOC', 'Bordeaux AOC', 'Bolgheri DOC', 'Gevrey-Chambertin 1-er Cru AOC', 'Volnay 1-er Cru AOC', 'Alto Adige Terlano DOC', 'Veneto IGT', 'Chassagne-Montrachet 1-er Cru AOC'
    ]

    category_list = ['Denominazione di Origine Controllata (DOC)', '1-er Grand Cru Classe', "Appellation d'Origine Controlee (AOC)", 'Denominacion de Origen Calificada (DOCa)', 'Grand Cru Classe',
    'Denominacion de Origen (DO)', 'Denominazione di Origine Controllata e Garantita (DOCG)', 'American Viticultural Areas (AVA)', '3-eme Grand Cru Classe', 'Indicazione Geografica Tipica (IGT)',
    '5-eme Grand Cru Classe', '4-eme Grand Cru Classe', '2-eme Grand Cru Classe', '1-er Grand Cru Superieur', 'Australian Geographical Indications (GI)', '1-er Grand Cru Classe "B"', 'Denominacion de Pago', 'Vino d’Italia', 'Indicacion Geografica (IG)'
    ]

    region_list = ['Аконкагуа', 'Апулия', 'Бордо', 'Бургундия', 'Венето', 'Долина Роны', 'Иудейские Холмы', 'Калифорния', 'Кастилья и Леон', 'Каталония', 'Ломбардия', 'Марке', 'Мендоса', 'Наварра',
    'Нижняя Австрия', 'Орегон', 'Пьемонт', 'Риоха', 'Сицилия', 'Токай', 'Тоскана', 'Трентино-Альто Адидже', 'Центральная Долина', 'Южная Австралия'
    ]
    ...
    df10 = df2
    df10['PartOfTheWorld'] = df10.apply(lambda row: int(row['Country']   in old_country_list),    axis = 1,  result_type = 'expand')
    df10['Decantation']  = df10.apply(lambda row: int(row['Decantation'] in decantation_list),    axis = 1,  result_type = 'expand')
    df10['Appellation']  = df10.apply(lambda row: int(row['Appellation'] in appellasion_list),    axis = 1,  result_type = 'expand')
    ...
    df10 = df10[['Price', 'Rating', 'Region', 'PartOfTheWorld','Appellation', 'Volume', 'ABV',  'GrapeSortNumber', 'Maturity', 'MinPotentialStorage']]
    ...
    gdf3 = gpd.GeoDataFrame(gdf3[['id', 'name','properties.name','properties.description', 'properties.CompanyMetaData.address', 'properties.CompanyMetaData.Hours.text',  'coordinates_lon','coordinates_lat']], geometry = gpd.points_from_xy(gdf3['coordinates_lon'], gdf3['coordinates_lat']))
